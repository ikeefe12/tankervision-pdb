...
- IMON uses R122 and reaches IO9 as `VIMON_USB`; U27 produces `PG_USB` for U26.P00
  and Q12/D22.
...
- IMON uses R124 and reaches IO3 as `VIMON_DC`; U28 produces `PG_DC` for U26.P01
  and Q13/D23.
...
The PG pins are not guaranteed source-presence indicators when their individual
input is absent and the comparator is powered from the other source's `+3V3_VBUS`.
See [comparators.md](comparators.md#input-pg-caveat).
